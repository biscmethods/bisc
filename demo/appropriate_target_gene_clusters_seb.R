...
save(cell_cluster_AC, file=path_AC)
rm(cell_cluster_AC)
gc()
...
save(cell_cluster_MES, file=path_MES)
rm(cell_cluster_MES)
gc()
...
save(cell_cluster_NPC, file=path_NPC)
rm(cell_cluster_NPC)
gc()
...
save(cell_cluster_OPC, file=path_OPC)
rm(cell_cluster_OPC)
gc()
...
for (i_cell_cluster in seq(n_cell_clusters)) {
  ...
  # remove cells and target genes
  keep_cells_split <- sample(c(1, 2), ncol(current_cell_cluster), prob = c(0.5, 0.5), replace = T)
  ...
  n_regulators <- sum(is_regulator)
  n_target_genes <- nrow(current_cell_cluster) - n_regulators
  ind_targetgenes <- which(c(rep(1, n_target_genes), rep(0, n_regulator_genes)) == 1)
  ind_reggenes <- which(c(rep(0, n_target_genes), rep(1, n_regulator_genes)) == 1)

  keep_target_genes_split <- sample(c(1, 2), n_target_genes, prob = c(0.10, 0.9), replace = T)
