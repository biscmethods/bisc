...
n_cells <- c(ncol(cell_cluster_AC), ncol(cell_cluster_OPC))
true_cluster_allocation <- factor(rep(1:n_cell_clusters, times = n_cells))
rm(n_cells)

penalization_lambdas <- c(0.1)
...
for (i in seq(1)) {
  ...
  OPC <- scregclust::scregclust(
    expression = cell_cluster_OPC,  # p rows of genes, n columns of cells
    genesymbols = 1:(n_target_genes + n_regulator_genes),  # Gene row numbers
    is_regulator = is_regulator,  # Vector indicating which genes are regulators
    n_cl = 2,
    penalization = penalization_lambdas,
  ...
  for (i_target_gene_cluster in seq(2)) {
    ...
    non_noise_regulator_genes <- non_noise_regulator_genes + OPC$results[[1]]$output[[1]]$reg_table[[i_target_gene_cluster]] != 0
  }
  ...
  non_noise_genes <- non_noise_genes + OPC$results[[1]]$output[[1]]$cluster  # Target genes that are in a cluster
  ...
  cell_cluster_OPC <- cell_cluster_OPC[non_noise_genes,]
}


biclust_input_data <- cbind(cell_cluster_AC, cell_cluster_OPC)
initial_clustering <- sample(true_cluster_allocation)
