...
RI_values_plaid <- data.frame(RI = unlist(sapply(all_res_bcplaid, function(x) x$RI_cell_clustering_biclustbiclust)))


# Calculate the summary statistics for each group
RI_summary_plaid <- RI_values_plaid %>%
  dplyr::summarize(
    Min = min(RI, na.rm = TRUE),
    Q1 = quantile(RI, 0.25, na.rm = TRUE),
    Median = median(RI, na.rm = TRUE),
...
latex_table <- xtable(RI_summary_plaid,  digits = 4)
latex_code <- print(latex_table,
                    type = "latex",
                    include.rownames = FALSE,
                    booktabs = TRUE)
